...
Раз уж мы вступили на тропу разработки ядра ОС, то об удобствах, доступных обычным нормисам, можно забыть. К этому относится и отладка сразу внутри **IDE**, как в **Visual Studio** или **Qt Creator**. Нам же остаётся, стиснув зубы, жёстко затерпеть и отлаживать прямо в консоли. Поначалу это может показаться неудобным, но со временем приходит смирение, и ты привыкаешь даже к такому… 😔

На самом деле проще делать отладку в консоли, чем разбираться, как подключиться из **IDE** к **GDB-серверу**. Но если кому-то интересно, вот [инструкция](https://doc.qt.io/qtcreator/creator-how-to-debug-remotely-gdb.html) для **Qt Creator**.
...
make debug
```
Если вы уже видели корневой [CMake файл](../../CMakeLists.txt), то давайте сразу перейдем к структуре комманды и флагов:
* `-cdrom ${CMAKE_BINARY_DIR}/${TARGET_NAME}.iso` – подключить iso образ как CD-ROM
...
Тем самым данная кастомная комманда собирает ISO, запускает его в QEMU, сразу ставит виртуальную машину на паузу и ждёт подключения GDB на порту 1234.
После запуска будет следующая катрина:
![start_qemu_rounded.png](img/start_qemu_rounded.png)

Теперь откроем новый терминал и подключемся к gdb севреру:
```bash
make gdb
```
Тут все проще чес с первой, у нас тут только один флаг:

* `-ex` – даёт понять что следующее вырожение заключенное в ковычках нужно сразу выполнить внутри gdb.

Как итог мы сразу подключаемся к удаленному серверу, и устонавливаем брейк-поинт в main функции и сразу продолжили выполнение виртуальной машины.
...
| `enable <номер>`                      | Включить отключенный breakpoint                      | `enable 2`                      |
...
| `finish`                              | Выполнить текущую функцию до конца и выйти из нее    | `finish`                        |
